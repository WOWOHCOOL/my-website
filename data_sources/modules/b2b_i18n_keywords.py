...
I18N_CTA_POSITIVE_PATTERNS: Dict[str, List[str]] = {
    ...
    'ru': [
        r'\b(?:Запросить\s+(?:расчёт|каталог|образцы|консультацию|прайс|цену|стоимость)|'
        r'Получить\s+(?:расчёт|каталог|консультацию|прайс|цену|предложение)|'
        r'Связаться\s+(?:с\s+нами)?|Написать\s+(?:в\s+)?WhatsApp|'
        r'Начать\s+(?:проект|сотрудничество)|Заказать\s+(?:образцы|звонок|консультацию|расчёт)|'

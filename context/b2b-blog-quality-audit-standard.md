...
**DOM 结构防护（CI 强制）**：正文中的普通 H3/H4 绝对不得携带 `.speakable` class，也不得通过继承或模板误带。`b2b_content_auditor.py` 的 DOM 解析器必须检测任意 `<h3 class="...speakable...">` / `<h4 class="...speakable...">`，一经发现即阻断发布。`.speakable` 只允许出现在 Hook wrapper 和 Key Takeaways TL;DR 两个位置；FAQ 答案使用独立的 `.faq-answer` selector，与 BlogPosting 的 `.speakable` 完全隔离。
...
| **答案侧** | 量化数据（数字+单位）**或**合规标准代号（如 IEC 62368-1, CE, RoHS, UN38.3）即判定为 B2B 深度达标；B2B 词汇（MOQ/FOB/lead time）作为补充信号，不作为硬性门槛 | 80% | AI 提取时答案是独立的引用单元，必须承载可验证的 B2B 深度，而不是为了凑分硬塞 `OEM`/`FOB` 导致文风僵硬 |
...
| 17 | **Static HTML Quality** | Featured image must not carry **hand-written uncompiled `srcset`/`sizes` relative paths**. `srcset` is allowed only when produced by the 11ty/SSG image pipeline, and every referenced variant file must be verified to exist. Also checks `fetchpriority`, `.speakable`, TOC anchor bugs. Sub-rule: no RESPUESTA RÁPIDA / SCHNELLANTWORT / Quick Answer block (present = -25) | Hand-written `srcset` relative path = -15; missing variant file = -15; per violation |
...
| **Featured Image** | No hand-written uncompiled `srcset`/`sizes` relative paths. If `srcset` is generated by the 11ty/SSG image pipeline, every referenced variant file must exist on disk (no `-800`/`-1200` 404s). `fetchpriority="high"` optional. | 🤖 |
...
[ ] Featured Image has no hand-written srcset/sizes; if srcset is SSG-generated, all variant files verified present, and fetchpriority="high"?
...
- Featured image must not carry hand-written uncompiled `srcset`/`sizes`; SSG-generated `srcset` is allowed only when every variant file is verified present + explicit `width`/`height` attributes
...
- [ ] No hand-written `srcset`/`sizes` on featured image; if SSG-generated, verify every variant file exists (`-800`/`-1200`) to prevent AI crawler 404s
